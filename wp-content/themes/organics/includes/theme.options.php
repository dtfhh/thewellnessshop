...
if ( !function_exists( 'organics_options_settings_theme_setup' ) ) {
	...
	function organics_options_settings_theme_setup() {
		...
		"footer_copyright" => array(
					...
					"std" => "Organics &copy; {Y} All Rights Reserved ",
					"rows" => "10",
					"type" => "editor"),
		...
		'responsive_layouts' => array(
            ...
            'privacy_text' => array(
                "title" => esc_html__("Text with Privacy Policy link", 'organics'),
                "desc"  => wp_kses_data( __("Specify text with Privacy Policy link for the checkbox 'I agree ...'", 'organics') ),
                "std"   => wp_kses_post( __( 'I agree that my submitted data is being collected and stored.', 'organics') ),
                "type"  => "text"
            ),
		...
		"info_socials_4" => array(
				"title" => esc_html__('Login via Social network', 'organics'),
				"desc" => esc_html__("Settings for the Login via Social networks", 'organics'),
				"type" => "info"),

		"social_login" => array(
				"title" => esc_html__('Social plugin shortcode',  'organics'),
				"desc" => esc_html__('Social plugin shortcode like [plugin_shortcode]',  'organics'),
				"divider" => false,
				"std" => "",
				"type" => "text"),
		...
		"info_contact_3" => array(
					...
					"desc" => wp_kses( __('What function you want to use for sending mail: the built-in Wordpress or standard PHP function? Attention! Some plugins may not work with one of them and you always have the ability to switch to alternative.', 'organics'), $ORGANICS_GLOBALS['allowed_tags'] ),
					"type" => "info"),
		...
		"info_service_3" => array(
			"title" => esc_html__('API Keys', 'organics'),
			"desc" => wp_kses_data( __('API Keys for some Web services', 'organics') ),
			"type" => "info"),
		'api_google' => array(
			"title" => esc_html__('Google API Key', 'organics'),
			"desc" => wp_kses_data( __("Insert Google API Key for browsers into the field above to generate Google Maps", 'organics') ),
			"std" => "",
			"type" => "text")
...
if ( !function_exists( 'organics_options_reset' ) ) {
	function organics_options_reset($clear=true) {
        $theme_slug = str_replace(' ', '_', trim(organics_strtolower(get_stylesheet())));
        $option_name = 'organics_' . trim($theme_slug) . '_options_reset';
        if ( get_option($option_name, false) === false ) {
            if ($clear) {
                ...
                $txt = organics_fgc(organics_storage_get('demo_data_url') . 'default/templates_options.txt');
                if (!empty($txt)) {
