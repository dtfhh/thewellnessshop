...
function organics_skin_customizer(option, val) {
	"use strict";
	...
	if (option == 'body_style') {
		...
		var params = ORGANICS_GLOBALS['co_add_params']!=undefined ? ORGANICS_GLOBALS['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#organics-customizer-styles-'+option).length > 0 ? jQuery('#organics-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="organics-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
