...
if ( !function_exists( 'organics_template_list_theme_setup' ) ) {
	add_action( 'organics_action_before_init_theme', 'organics_template_list_theme_setup', 1 );
	function organics_template_list_theme_setup() {
		organics_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'organics')
			));
	}
}

// Template output
if ( !function_exists( 'organics_template_list_output' ) ) {
	function organics_template_list_output($post_options, $post_data) {
