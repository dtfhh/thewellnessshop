...
if ( !function_exists( 'organics_template_plain_output' ) ) {
	function organics_template_plain_output($post_options, $post_data) {
		?>
		<div class="post_item sc_blogger_item sc_plain_item<?php organics_show_layout($post_options['number'] == $post_options['posts_on_page'] && !organics_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>" style="background-image: url(<?php echo (wp_get_attachment_url( get_post_thumbnail_id($page->ID) )); ?>);">
