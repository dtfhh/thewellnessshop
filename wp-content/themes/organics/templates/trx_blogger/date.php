...
if ( !function_exists( 'organics_template_date_output' ) ) {
	function organics_template_date_output($post_options, $post_data) {
		...
		<div class="post_item sc_blogger_item
			<?php organics_show_layout($post_options['number'] == $post_options['posts_on_page'] && !organics_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>"
			<?php organics_show_layout($post_options['dir'] == 'horizontal' ? ' style="width:'.(100/$post_options['posts_on_page']).'%"' : ''); ?>>
			<div class="sc_blogger_date">
				<span class="day_month"><?php organics_show_layout($post_data['post_date_part1']); ?></span>
				<span class="year"><?php organics_show_layout($post_data['post_date_part2']); ?></span>
			</div>
			...
			<div class="post_content">
				<h6 class="post_title sc_title sc_blogger_title">
					...
					<?php organics_show_layout($post_data['post_title']); ?>
		...
		?>
			<div class="load_more"<?php organics_show_layout($post_options['dir'] == 'horizontal' ? ' style="width:'.(100/$post_options['posts_on_page']).'%"' : ''); ?>></div>
		<?php
		}
	}
}
