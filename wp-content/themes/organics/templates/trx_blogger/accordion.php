...
if ( !function_exists( 'organics_template_accordion_output' ) ) {
	function organics_template_accordion_output($post_options, $post_data) {
		?>
		<div class="post_item sc_blogger_item sc_accordion_item<?php organics_show_layout($post_options['number'] == $post_options['posts_on_page'] && !organics_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>">
			
			<h5 class="post_title sc_title sc_blogger_title sc_accordion_title"><span class="sc_accordion_icon sc_accordion_icon_closed icon-plus"></span><span class="sc_accordion_icon sc_accordion_icon_opened icon-minus"></span><?php organics_show_layout($post_data['post_title']); ?></h5>
			...
			<div class="post_content sc_accordion_content">
				...
				if (organics_param_is_on($post_options['info'])) {
					...
					<div class="post_info">
						...
						<span class="post_info_item post_info_counters">
							<?php organics_show_layout($post_options['orderby']=='comments' || $post_options['counters']=='comments' ? esc_html__('Comments', 'organics') : esc_html__('Views', 'organics')); ?>
							<span class="post_info_counters_number"><?php organics_show_layout($post_options['orderby']=='comments' || $post_options['counters']=='comments' ? $post_data['post_comments'] : $post_data['post_views']); ?></span>
						</span>
					</div>
					<?php
				}
				...
				if (!organics_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
					organics_show_layout(organics_do_shortcode('[trx_button link="'.esc_url($post_data['post_link']).'"]'.($post_options['readmore']).'[/trx_button]'));
				}
				?>
