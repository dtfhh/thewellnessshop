...
if ( !function_exists( 'organics_template_services_3_theme_setup' ) ) {
	add_action( 'organics_action_before_init_theme', 'organics_template_services_3_theme_setup', 1 );
	function organics_template_services_3_theme_setup() {
		organics_add_template(array(
			'layout' => 'services-3',
			'template' => 'services-3',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 3/', 'organics'),
...
// Template output
if ( !function_exists( 'organics_template_services_3_output' ) ) {
	function organics_template_services_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php organics_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					organics_show_layout(organics_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
				} else {
					?>
