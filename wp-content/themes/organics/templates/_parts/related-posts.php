...
if (organics_get_custom_option("show_post_related") == 'yes' ){
    if(function_exists('organics_exists_woocommerce') && organics_exists_woocommerce() && (is_cart() || is_checkout() || is_account_page())){
        return;
    }
	...
	if ($number > 0) {
		...
		$need_dummy = false;
		?>
		<section class="related_wrap<?php organics_show_layout(($columns>1 ? '' : ' scroll_wrap') . esc_attr(organics_get_template_property('related', 'container_classes'))); ?>">
