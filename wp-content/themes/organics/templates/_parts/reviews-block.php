...
// Reviews block
$reviews_markup = '';
if ( !empty($avg_author) || !empty($avg_users) ) {
	...
	if (!$reviews_first_author || !$reviews_second_hide) {
		...
		$field["descr"] = wp_kses( sprintf(__("Summary rating from <b>%s</b> user's marks.", 'organics'), $users) 
			...
			.(!isset($_COOKIE['organics_votes']) || organics_strpos($_COOKIE['organics_votes'], ','.($post_data['post_id']).',')===false
				? esc_html__('You can set own marks for this article - just click on stars above and press "Accept".', 'organics')
                                            : esc_html__('Thanks for your vote!', 'organics')
                                      ), organics_storage_get('allowed_tags') );
		$field["accept"] = $allow_user_marks;
		$output .= '<div id="users_marks" class="sc_tabs_content'.(!$output ? ' block_is_visible' : '') . '">' . trim(organics_reviews_get_markup($field, $marks, $allow_user_marks, false, !$reviews_first_author)) . '</div>';
	}
	$reviews_markup .= $output . '</div>';
	if ($allow_user_marks) {
		wp_enqueue_script('jquery-ui-draggable', false, array('jquery', 'jquery-ui-core'), null, true);

		organics_storage_set_array('js_vars', 'reviews_allow_user_marks', $allow_user_marks);
		organics_storage_set_array('js_vars', 'reviews_max_level', $max_level);
		organics_storage_set_array('js_vars', 'reviews_levels', trim(organics_get_theme_option('reviews_criterias_levels')));
		organics_storage_set_array('js_vars', 'reviews_vote', (isset($_COOKIE['organics_votes']) ? $_COOKIE['organics_votes'] : ''));
		organics_storage_set_array('js_vars', 'reviews_marks', explode(',', $marks));
		organics_storage_set_array('js_vars', 'reviews_users', max(0, $users));
		organics_storage_set_array('js_vars', 'post_id', $post_data['post_id']);
	}
