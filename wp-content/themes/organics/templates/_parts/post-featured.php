<?php
					if (isset($post_data['post_video']) and $post_data['post_video']) {
						organics_show_layout(organics_get_video_frame($post_data['post_video'], $post_data['post_video_image'] ? $post_data['post_video_image'] : $post_data['post_thumb']));
						...
						if (organics_get_custom_option('substitute_audio')=='no' || !organics_in_shortcode_blogger(true))
							organics_show_layout(organics_get_audio_frame($post_data['post_audio'], $post_data['post_audio_image'] ? $post_data['post_audio_image'] : $post_data['post_thumb_url']));
						else
							organics_show_layout($post_data['post_audio']);
						...
						organics_show_layout($post_data['post_gallery']);
					}
?>