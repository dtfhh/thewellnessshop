...
if ( !function_exists( 'organics_template_single_portfolio_output' ) ) {
	function organics_template_single_portfolio_output($post_options, $post_data) {
		...
		if ($show_title) {
			?>
            <h1 itemprop="<?php organics_show_layout($avg_author > 0 || $avg_users > 0 ? 'itemReviewed' : 'headline'); ?>" class="post_title entry-title"><?php organics_show_layout($post_data['post_title']); ?></h1>
			<?php
		}
