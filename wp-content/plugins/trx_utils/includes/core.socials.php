...
// Theme init
if (!function_exists('organics_socials_theme_setup')) {
	add_action( 'organics_action_before_init_theme', 'organics_socials_theme_setup' );
	function organics_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'organics_facebook_og_tags', 5 );

		} else {
            // Add extra fields in the user profile
            add_action( 'show_user_profile',		'organics_add_fields_in_user_profile' );
            add_action( 'edit_user_profile',		'organics_add_fields_in_user_profile' );

            // Save / update additional fields from profile
            add_action( 'personal_options_update',	'organics_save_fields_in_user_profile' );
            add_action( 'edit_user_profile_update',	'organics_save_fields_in_user_profile' );
        }


        // List of social networks for site sharing and user profiles
        $ORGANICS_GLOBALS['share_links'] = array(
            ...
            'postila' =>		'http://postila.ru/publish/?url={link}&agregator=organic-store',
...
if (!function_exists('organics_add_share_link')) {
	function organics_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		global $ORGANICS_GLOBALS;
		$ORGANICS_GLOBALS['share_links'] = array_merge( $ORGANICS_GLOBALS['share_links'], $soc );
	}
}

// Return (and show) share social links
if (!function_exists('organics_show_share_links')) {
	function organics_show_share_links($args) {
		if ( organics_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => organics_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => organics_get_custom_option('show_share'),				// share block direction
			'caption' => organics_get_custom_option('share_caption'),				// share block caption
			'share' => organics_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = empty($soc['url']) && !empty($ORGANICS_GLOBALS['share_links'][$sn]) ? $ORGANICS_GLOBALS['share_links'][$sn] : $soc['url'];
				...
				if (isset($url['attributes']) && is_array($url['attributes'])) {
					foreach($url['attributes'] as $k2=>$v2) {
						$v2 = str_replace(
							array('{id}', '{link}', '{title}', '{descr}', '{image}'),
							array(
								$k2=='href' ? urlencode($args['post_id']) : $args['post_id'],
								$k2=='href' ? urlencode($args['post_link']) : $args['post_link'],
								$k2== urlencode(strip_tags($args['post_title'])),
								$k2==urlencode(strip_tags($args['post_descr'])),
								$k2=='href' ? urlencode($args['post_thumb']) : $args['post_thumb']
							),
							$v2);
						$output .= " {$k2}=\"" . ($k2=='href' ? esc_url($v2) : esc_attr($v2)) . '"';
					}
				}
				...
				$output .=  '>'
					...
					. ($args['type']=='drop' ? '<i>' . trim(organics_strtoproper($sn)) . '</i>' : '')
					. '</div>';
...
if (!function_exists('organics_prepare_socials')) {
	function organics_prepare_socials($list, $style='') {
		if (empty($style)) $style = organics_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || organics_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : organics_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('organics_get_twitter_data')) {
	function organics_get_twitter_data($cfg) {
		return function_exists('organics_twitter_acquire_data') 
				? organics_twitter_acquire_data(array(
						'mode'            => 'user_timeline',
...
if (!function_exists('organics_prepare_twitter_text')) {
	function organics_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('organics_get_twitter_followers')) {
	function organics_get_twitter_followers($cfg) {
		$data = organics_get_twitter_data($cfg); 
...
if (!function_exists('organics_get_facebook_likes')) {
	function organics_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(organics_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (organics_strpos($headers[0], '200')) {
			$json = organics_fgc($url);
...
if (!function_exists('organics_facebook_og_tags')) {
	function organics_facebook_og_tags() {
		global $post;
		if ( !is_singular() || organics_get_global('blog_streampage')) return;
...
if (!function_exists('organics_get_feedburner_counter')) {
	function organics_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(organics_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (organics_strpos($headers[0], '200')) {
			$xml = organics_fgc($url);
...
if (!function_exists('organics_add_fields_in_user_profile')) {
    function organics_add_fields_in_user_profile( $user ) {
        ?>
        <h3><?php esc_html_e('User Position', 'organics'); ?></h3>
        <table class="form-table">
            <tr>
                <th><label for="user_position"><?php esc_html_e('User position', 'organics'); ?>:</label></th>
                <td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
                    <span class="description"><?php esc_html_e('Please, enter your position in the company', 'organics'); ?></span>
                </td>
            </tr>
        </table>

        <h3><?php esc_html_e('Social links', 'organics'); ?></h3>
        <table class="form-table">
            <?php
            ...
            if (is_array($social_list) && count($social_list) > 0) {
                foreach ($social_list as $soc) {
                    ...
                    if (!empty($sn)) {
                        ?>
                        <tr>
                            <th><label for="user_<?php echo esc_attr($sn); ?>"><?php organics_show_layout(organics_strtoproper($sn)); ?>:</label></th>
                            <td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
                                <span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'organics'), organics_strtoproper($sn)); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
...
if (!function_exists('organics_save_fields_in_user_profile')) {
    function organics_save_fields_in_user_profile( $user_id ) {
