 ...
 */
add_action( 'widgets_init', 'organics_widget_twitter_load' );

/**
 * Register our widget.
 * 'Twitter_Widget' is the widget class used below.
 */
function organics_widget_twitter_load() {
	register_widget( 'organics_widget_twitter' );
}

/**
 * Twitter Widget class.
 */
class organics_widget_twitter extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'organics_widget_twitter' );

		/* Create the widget. */
		parent::__construct( 'organics_widget_twitter', esc_html__('Organics - Twitter', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $tweet) {
				...
				$output .= '<li class="theme_text' . ($cnt==$twitter_count-1 ? ' last' : '') . '"><a href="' . esc_url('https://twitter.com/'.($twitter_username)) . '" class="username" target="_blank">@' . ($tweet['user']['screen_name']) . '</a> ' . force_balance_tags(organics_prepare_twitter_text($tweet)) . '</li>';
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => esc_html__('Last Twitter Updates', 'trx_utils') );
		$instance = wp_parse_args( (array) $instance, $defaults ); 
		$title = isset($instance['title']) ? $instance['title'] : '';
		$twitter_username = isset($instance['twitter_username']) ? $instance['twitter_username'] : organics_get_theme_option('twitter_username');
		$twitter_consumer_key = isset($instance['twitter_consumer_key']) ? $instance['twitter_consumer_key'] : organics_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = isset($instance['twitter_consumer_secret']) ? $instance['twitter_consumer_secret'] : organics_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = isset($instance['twitter_token_key']) ? $instance['twitter_token_key'] : organics_get_theme_option('twitter_token_key');
		$twitter_token_secret = isset($instance['twitter_token_secret']) ? $instance['twitter_token_secret'] : organics_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, intval(isset($instance['twitter_count']) ? $instance['twitter_count'] : organics_get_theme_option('twitter_count')));
		?>
