 ...
 */
add_action( 'widgets_init', 'organics_widget_qrcode_load' );

/**
 * Register our widget.
 */
function organics_widget_qrcode_load() {
	register_widget('organics_widget_qrcode');
}

/**
 * QRCode Widget class.
 */
class organics_widget_qrcode extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'organics_widget_qrcode');

		/* Create the widget. */
		parent::__construct( 'organics_widget_qrcode', esc_html__('Organics - QRCode generator', 'trx_utils'), $widget_ops, $control_ops );
	...
	function form($instance) {
		...
		//wp_enqueue_style('color-picker',        organics_get_file_url('js/colorpicker/colorpicker.css'), array(), null);
		//wp_enqueue_script('color-picker',       organics_get_file_url('js/colorpicker/colorpicker.js'), array('jquery'),null,true);

		/* Set up some default widget settings. */
		$address = explode(',', organics_get_theme_option('user_address'));
		$defaults = array(
			'title' => '', 
		...
		<div class="widget_qrcode">
        	<div class="qrcode_tabs">
                <ul class="tabs">
                    ...
                    <li><a href="#tab_fields" onmousedown="organics_qrcode_init()"><?php esc_html_e('Personal Data', 'trx_utils'); ?></a></li>
                    <li><a href="#tab_text" onmousedown="organics_qrcode_init()"><?php esc_html_e('Any Text', 'trx_utils'); ?></a></li>
                </ul>
                <div id="tab_settings" class="tab_content tab_settings">
                    <p>
                        ...
                        <input class="fld_title" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']); ?>" style="width:100%;" />
                    </p>
                    <p>
                        <label><?php esc_html_e('Show as QR Code:', 'trx_utils'); ?></label><br />
                        <input class="fld_show_what" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="1" type="radio" <?php echo ($instance['show_what']==1 ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1"> <?php esc_html_e('Personal VCard', 'trx_utils'); ?></label>
                        <input class="fld_show_what" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="0" type="radio" <?php echo ($instance['show_what']==0 ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0"> <?php esc_html_e('Any text', 'trx_utils'); ?></label>
                    </p>
                    <p>
                        <input class="fld_show_personal" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_personal')); ?>" name="<?php echo esc_attr($this->get_field_name('show_personal')); ?>" value="1" type="checkbox" <?php echo ($instance['show_personal']==1 ? 'checked="checked"' : ''); ?> />
                    ...
                    <p>
                        ...
                        <input onmousedown="organics_qrcode_init()" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('width')); ?>" name="<?php echo esc_attr($this->get_field_name('width')); ?>" value="<?php echo esc_attr($instance['width']); ?>" style="width:100%;" class="fld_width" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php esc_html_e('Color:', 'trx_utils'); ?></label>
                        <input onmousedown="organics_qrcode_init()" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('color')); ?>" name="<?php echo esc_attr($this->get_field_name('color')); ?>" value="<?php echo esc_attr($instance['color']); ?>" style="width:100%; background-color:<?php echo esc_attr($instance['color']); ?>" class="iColorPicker fld_color" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('bg')); ?>"><?php esc_html_e('Bg color:', 'trx_utils'); ?></label>
                        <input onmousedown="organics_qrcode_init()" onfocus="organics_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('bg')); ?>" name="<?php echo esc_attr($this->get_field_name('bg')); ?>" value="<?php echo esc_attr($instance['bg']); ?>" style="width:100%; background-color:<?php echo esc_attr($instance['bg']); ?>" class="iColorPicker fld_bg" />
                    </p>
                </div>
...
<?php } ?>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function(){
				organics_qrcode_init();
				organics_color_picker();
            });
			function organics_qrcode_init() {
            ...
            function organics_qrcode_update(widget) {
				organics_qrcode_show(widget, {
			...
			function organics_qrcode_show(widget, vc, opt) {
				if (opt.show_what==1) {
			...
			if (!window.organics_set_color_picker) {
				function organics_set_color_picker(id_picker) {
