 ...
 */
add_action( 'widgets_init', 'organics_widget_popular_posts_load' );

/**
 * Register our widget.
 */
function organics_widget_popular_posts_load() {
	register_widget('organics_widget_popular_posts');
}

/**
 * Most popular and commented Widget class.
 */
class organics_widget_popular_posts extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'organics_widget_popular_posts');

		/* Create the widget. */
		parent::__construct( 'organics_widget_popular_posts', esc_html__('Organics - Most Popular & Commented Posts', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? organics_get_theme_option('blog_counters') : '';
		...
		if ( !empty($titles) ) {
            ...
            organics_show_layout($before_widget);
			
			/* Display the widget title if one was input (before and after defined by themes). */
            if ($title) organics_show_layout($title, $before_title, $after_title);
            ...
            organics_show_layout($after_widget);
		}
	}
