 ...
 */
add_action( 'widgets_init', 'organics_widget_socials_load' );

/**
 * Register our widget.
 */
function organics_widget_socials_load() {
	register_widget( 'organics_widget_socials' );
}

/**
 * flickr Widget class.
 */
class organics_widget_socials extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'organics_widget_socials' );

		/* Create the widget. */
		parent::__construct( 'organics_widget_socials', esc_html__('Organics - Show logo and social links', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? organics_do_shortcode($instance['text']) : '';
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					if ($logo_image=='')	$logo_image = $ORGANICS_GLOBALS['logo'];
					if ($logo_text=='')		$logo_text = $ORGANICS_GLOBALS['logo_text'];
					if ($logo_slogan=='')	$logo_slogan = $ORGANICS_GLOBALS['logo_slogan'];
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'logo_image'=>'', 'logo_text'=>'', 'show_logo' => '1', 'show_icons' => '1', 'text'=>'', 'description' => esc_html__('Show logo and social icons', 'trx_utils') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php esc_html_e('Logo image:<br />(if empty - use logo from Theme Options)', 'trx_utils'); ?></label>
            ...
            <?php
			organics_show_layout(organics_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
