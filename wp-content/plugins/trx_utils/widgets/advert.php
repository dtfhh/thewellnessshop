 ...
 */
add_action( 'widgets_init', 'organics_widget_advert_load' );

/**
 * Register our widget.
 */
function organics_widget_advert_load() {
	register_widget( 'organics_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class organics_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => esc_html__('Advertisement block', 'trx_utils') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'organics_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'organics_widget_advert', esc_html__('Organics - Advertisement block', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				echo force_balance_tags(organics_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => esc_html__('Advertisement block', 'trx_utils') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php esc_html_e('Image source URL:<br />(leave empty if you paste advert code)', 'trx_utils'); ?></label>
            ...
            <?php
            organics_show_layout(organics_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php esc_html_e('Image link URL:<br />(leave empty if you paste advert code)', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php esc_html_e('or paste Advert Widget HTML Code:', 'trx_utils'); ?></label>
