 ...
 */
add_action( 'widgets_init', 'organics_widget_recent_posts_load' );

/**
 * Register our widget.
 */
function organics_widget_recent_posts_load() {
	register_widget('organics_widget_recent_posts');
}

/**
 * Recent posts Widget class.
 */
class organics_widget_recent_posts extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'organics_widget_recent_posts');

		/* Create the widget. */
		parent::__construct( 'organics_widget_recent_posts', esc_html__('Organics - Recent Posts', 'trx_utils'), $widget_ops, $control_ops );

		// Add thumb sizes into list
		organics_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'h_crop' => 75, 'title'=>esc_html__('Widgets', 'trx_utils') ) );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_author']!='hide' ? $instance['show_author'] : '';
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), organics_get_list_blog_counters(false));
		$categories = organics_get_list_terms(false, organics_get_taxonomy_categories_by_post_type($post_type));
		?>
		...
		<p>
			...
			<select id="<?php echo esc_attr($this->get_field_id('post_type')); ?>" name="<?php echo esc_attr($this->get_field_name('post_type')); ?>" style="width:100%;" onchange="organics_admin_change_post_type(this);">
			<?php
