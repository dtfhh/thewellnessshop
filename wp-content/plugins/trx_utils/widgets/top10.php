 ...
 */
add_action( 'widgets_init', 'organics_widget_top10_load' );

/**
 * Register our widget.
 */
function organics_widget_top10_load() {
	register_widget('organics_widget_top10');
}

/**
 * Top10 Widget class.
 */
class organics_widget_top10 extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'organics_widget_top10');

		/* Create the widget. */
		parent::__construct( 'organics_widget_top10', esc_html__('Organics - Top 10 Posts', 'trx_utils'), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		...
		if (count($tabs) > 0) {
			...
			echo count($tabs) == 1 ? $tabs[0]['content'] : organics_do_shortcode('[trx_tabs style="2"][trx_tab title="'.esc_attr($tabs[0]['title']).'"]'.($tabs[0]['content']).'[/trx_tab][trx_tab title="'.esc_attr($tabs[1]['title']).'"]'.($tabs[1]['content']).'[/trx_tab][/trx_tabs]');
