...
if (function_exists('vc_map_get_attributes')) {						// New VC
	 ...
	 * Shortcode class
	 * @var $this WPBakeryShortCode_VC_Ro
	 */
	$output = $after_output = '';
    ...
    // Add Organics color scheme in the row
	if ($scheme && !organics_param_is_off($scheme) && !organics_param_is_inherit($scheme)) $css_classes[] = 'scheme_'.esc_attr($scheme);
	...
	$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'vc_row wpb_row ' . ( $this->settings( 'base' ) === 'vc_row_inner' ? 'vc_inner ' : '' ) . (empty(vc_settings()->get( 'row_css_class' )) ? 'vc_row-fluid' : vc_settings()->get( 'row_css_class' ))." " . $el_class . vc_shortcode_custom_css_class( $css, ' ' ), $this->settings['base'], $atts );
	
	// Add Organics color scheme in the row
	if ($scheme && !organics_param_is_off($scheme) && !organics_param_is_inherit($scheme)) $css_class .= ' scheme_'.esc_attr($scheme);
