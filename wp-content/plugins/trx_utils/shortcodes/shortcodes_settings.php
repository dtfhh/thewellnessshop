...
if ( !function_exists( 'organics_get_sc_param' ) ) {
    function organics_get_sc_param($prm) {
        return organics_storage_get_array('sc_params', $prm);
    }
}

// Set sc_param value
if ( !function_exists( 'organics_set_sc_param' ) ) {
    function organics_set_sc_param($prm, $val) {
        organics_storage_set_array('sc_params', $prm, $val);
    }
}

// Add sc settings in the sc list
if ( !function_exists( 'organics_sc_map' ) ) {
    function organics_sc_map($sc_name, $sc_settings) {
        organics_storage_set_array('shortcodes', $sc_name, $sc_settings);
    }
}

// Add sc settings in the sc list after the key
if ( !function_exists( 'organics_sc_map_after' ) ) {
    function organics_sc_map_after($after, $sc_name, $sc_settings='') {
        organics_storage_set_array_after('shortcodes', $after, $sc_name, $sc_settings);
    }
}

// Add sc settings in the sc list before the key
if ( !function_exists( 'organics_sc_map_before' ) ) {
    function organics_sc_map_before($before, $sc_name, $sc_settings='') {
        organics_storage_set_array_before('shortcodes', $before, $sc_name, $sc_settings);
    }
}

// Compare two shortcodes by title
if ( !function_exists( 'organics_compare_sc_title' ) ) {
    function organics_compare_sc_title($a, $b) {
	...
	function organics_shortcodes_settings_theme_setup() {
		if (organics_shortcodes_is_used()) {
			...
			$ORGANICS_GLOBALS['sc_params'] = array(
                ...
                'categories'=> is_admin() && organics_get_value_gp('action')=='vc_edit_form' && substr(organics_get_value_gp('tag'), 0, 4)=='trx_' && isset($_POST['params']['post_type']) && $_POST['params']['post_type']!='post'
                    ? organics_get_list_terms(false, organics_get_taxonomy_categories_by_post_type($_POST['params']['post_type']))
                    : organics_get_list_categories(),
