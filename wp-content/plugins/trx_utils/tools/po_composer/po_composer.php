...
// Theme init
if (!function_exists('organics_po_composer_theme_setup')) {
	add_action( 'organics_action_after_init_theme', 'organics_po_composer_theme_setup' );		// Fire this action after load theme options
	function organics_po_composer_theme_setup() {
		if (is_admin() && current_user_can('manage_options') && organics_get_theme_option('admin_po_composer')=='yes') {
			new organics_po_composer();
		}
	}
}


class organics_po_composer {
	...
	function admin_menu_item() {
		...
		organics_admin_add_menu_item('management', array(
	...
	function build_page() {
		...
		$nonce = wp_create_nonce( get_admin_url() );
		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php esc_html_e('Organics .PO-files Composer', 'trx_utils'); ?></h2>
			<div class="po_composer_result">
				<?php if (!empty($this->error)) { ?>
				<div class="error">
					<p><?php organics_show_layout($this->error); ?></p>
				</div>
				<?php } ?>
				<?php if (!empty($this->error_update)) { ?>
					<div class="error">
						<p><?php organics_show_layout($this->error_update); ?></p>
					</div>
				<?php } ?>
				<?php if (!empty($this->success)) { ?>
				<div class="updated">
					<p><?php organics_show_layout($this->success); ?></p>
				</div>
				<?php } ?>
			</div>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
					...
					<fieldset class="po_composer_block_inner">
						...
						<div class="po_composer_fields">
							<div class="po_composer_field">
								...
								<select name="po_src" id="po_src"><?php organics_show_layout($options); ?></select>
					...
					<fieldset class="po_composer_block_inner">
						...
						<div class="po_composer_fields">
							<div class="po_composer_field">
								...
								<select name="po_src2" id="po_src2"><?php organics_show_layout($options2); ?></select>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo ($this->po_src=='' ? esc_html__('Upload', 'trx_utils') : esc_html__('Update', 'trx_utils')); ?></a>
		...
		if (!empty($this->po_src)) {
			...
			if (empty($this->error)) {
				...
				if (is_writeable($po_temp)) {
					organics_fpc($po_temp, $this->po_text);
				...
				if (is_writeable($mo_temp)) {
					organics_fpc($mo_temp, $this->mo_text);
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						organics_fpc($po_file, $this->po_text);
						$this->success = wp_kses( sprintf(__('Language file "<b>%s.po</b>" was updated!', 'trx_utils'), $name), array('b'=>array(), 'br'=>array()) );
						$mo_file = trim($dir).'/'.trim($name).'.mo';
						if (is_writeable($mo_file)) {
							organics_fpc($mo_file, $this->mo_text);
							$this->success .= '<br>' . wp_kses( sprintf(__('Language file "<b>%s.mo</b>" was generated!', 'trx_utils'), $name), array('b'=>array(), 'br'=>array()) );
						}
					} else {
						$tjis->error_update = wp_kses( sprintf(__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'trx_utils'), $name), array('b'=>array(), 'br'=>array()) );
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				$rez['data'] = isset($_FILES['po_file'.($suffix)]['tmp_name']) && file_exists($_FILES['po_file'.($suffix)]['tmp_name']) ? organics_fga($_FILES['po_file'.($suffix)]['tmp_name']) : '';
				if (empty($rez['data'])) {
			...
			// or load file from 'languages' folder
				...
				$name = organics_esc(substr($_POST['po_src'.($suffix)], strpos($_POST['po_src'.($suffix)], '_')+1));
				$rez['data'] = file_exists(($dir).'/'.($name).'.po') ? organics_fga(($dir).'/'.($name).'.po') : '';
				if (empty($rez['data'])) {
