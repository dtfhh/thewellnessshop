...
class organics_emailer {
	...
	function prepare_js() { 
		...
		<script type="text/javascript">
			...
			var ORGANICS_EMAILER_ajax_error = "<?php esc_html_e('Invalid server answer!', 'trx_utils'); ?>";
		</script>
		<?php 
	}
	...
	function build_page() {
		...
		<div class="trx_emailer">
			<h2 class="trx_emailer_title"><?php esc_html_e('Organics Emailer', 'trx_utils'); ?></h2>
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						...
						<div class="trx_emailer_fields">
							...
							<?php if ( isset($mail) and ($mail=='wp_mail')) { ?>
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('organics_emailer_subscribers', $subscribers);
					organics_set_system_message(esc_html__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'trx_utils'), 'success');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url('/') );
				} else if ($subscribers[$group][$email] != '') {
					organics_set_system_message(esc_html__('Bad confirmation code!', 'trx_utils'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url('/') );
				} else {
					organics_set_system_message(esc_html__('E-mail address already exists in the subscribers list!', 'trx_utils'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url('/') );
				}
			}
